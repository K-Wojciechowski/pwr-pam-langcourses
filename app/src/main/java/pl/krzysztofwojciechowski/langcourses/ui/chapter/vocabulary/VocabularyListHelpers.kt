...
fun getBestVocabularySpan(vocabularyListItems: List<VocabularyListItem>): Int {
    val maxLength = vocabularyListItems.filter { it.type == VocabularyListItemType.ENTRY }.map {
        kotlin.math.max(
            it.original.length,
            it.translated.length
        )
    }.max() ?: Int.MAX_VALUE
...
class VocabularySpanSizeLookup(
    private val vocabularyListItems: List<VocabularyListItem>,
